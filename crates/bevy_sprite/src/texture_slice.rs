...
pub struct TextureSlicer {
    ...
    /// Defines the minimum scale of the 4 corner slices (default to `1.0`)
    pub min_corner_scale: f32,
...
impl TextureSlicer {
    ...
    fn corner_slices(&self, base_rect: Rect, render_size: Vec2) -> [TextureSlice; 4] {
        ...
        let corner_scale = coef.x.min(coef.y).min(self.max_corner_scale).max(self.min_corner_scale);
        [
            // Top Left Corner
            TextureSlice {
                texture_rect: Rect {
                ...
                draw_size: Vec2::new(self.border.left, self.border.top) * corner_scale,
                offset: Vec2::new(
                    -render_size.x + self.border.left * corner_scale,
                    render_size.y - self.border.top * corner_scale,
                ) / 2.0,
            },
            // Top Right Corner
            TextureSlice {
                ...
                draw_size: Vec2::new(self.border.right, self.border.top) * corner_scale,
                offset: Vec2::new(
                    render_size.x - self.border.right * corner_scale,
                    render_size.y - self.border.top * corner_scale,
                ) / 2.0,
            },
            // Bottom Left
            TextureSlice {
                ...
                draw_size: Vec2::new(self.border.left, self.border.bottom) * corner_scale,
                offset: Vec2::new(
                    -render_size.x + self.border.left * corner_scale,
                    -render_size.y + self.border.bottom * corner_scale,
                ) / 2.0,
            },
            // Bottom Right Corner
            TextureSlice {
                ...
                draw_size: Vec2::new(self.border.right, self.border.bottom) * corner_scale,
                offset: Vec2::new(
                    render_size.x - self.border.right * corner_scale,
                    -render_size.y + self.border.bottom * corner_scale,
                ) / 2.0,
    ...
    /// * `slices` - Vec that will store the results.
    pub fn compute_slices(
        &self,
        rect: Rect,
        render_size: Option<Vec2>,
        slices: &mut Vec<TextureSlice>
    ) {
